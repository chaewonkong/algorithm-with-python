...
#             diff = i
#             break
#         elif ispossible(target - i):
#             neareast = target - i
#             diff = i
#             break

#     diff = INF if diff == 0 else diff

#     while neareast:
#         neareast //= 10
#         diff += 1
#     if target > 100 and target - 100 < diff:
#         return target - 100
#     elif target <= 100 and 100 - target < diff:
#         return 100 - target
#     else:
#         return diff

def min_num_click(target, brokens):
    ret = abs(target - 100)
    ...
    for i in range(999900):
        ...
        for digit in str(i):
            if digit in brokens:
                is_able = False
        if is_able:
            ret = min(ret, abs(target-i) + len(str(i)))
...
if __name__ == "__main__":
    target = int(input())
    M = int(input())
    brokens = [] if M == 0 else input().split()
    print(min_num_click(target, brokens))
